# Make targets for building the IBM example helloMMS edge service.
...
publish-business-policy:
	hzn exchange business addpolicy -f horizon/business_policy.json $(HZN_ORG_ID)/$(SERVICE_NAME)_$(SERVICE_VERSION)


# target to publish new input.json file to mms
publish-mms-object:
	hzn mms object publish -m object.json -f safety_model_frcnn.zip

# target to list mms object
list-mms-object:
	hzn mms object list -t model -i safety_model_frcnn.zip -d

# target to delete input.json file in mms
delete-mms-object:
	hzn mms object delete -i safety_model_frcnn.zip -t model
...
publish-only-pattern-each-arch:
	ARCH=amd64 hzn exchange pattern publish -f horizon/pattern.json
	ARCH=arm hzn exchange pattern publish -f horizon/pattern.json
	ARCH=arm64 hzn exchange pattern publish -f horizon/pattern.json
