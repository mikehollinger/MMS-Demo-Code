...
OBJECT_ID=safety_model_frcnn.zip
PATH_TO_MODEL=/config/dropins
...
cp $PATH_TO_MODEL/model.zip $PATH_TO_MODEL/model.zip.original
...
while true; do
    ...
    if [[ "$HTTP_CODE" == '200' && "$OBJ_ID" == $OBJECT_ID ]]; then
	    ...
	    echo "PROCESS: moving new model to /config/dropins/ "
	    cp $OBJECT_ID $PATH_TO_MODEL/model.zip
